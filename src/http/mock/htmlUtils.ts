import type { IApi, IRoute, webpack } from '@umijs/types';
...
type IChunkMap = Record<string, string>;
  ...
  class Html extends api.Html {
    ...
    async getContent(args: IGetContentArgs): Promise<string> {
      ...
      if (api.config.exportStatic && api.config.exportStatic?.dynamicRoot) {
        ...
        publicPathStr =
          'location.protocol + \'//\' + location.hostname + (location.port ? \':\' + location.port : \'\') + window.routerBase';
      }
